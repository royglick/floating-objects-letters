...
/* spatial hash — head/next linked lists, zero allocation per frame.
   The cell is re-sized every frame to the flock's ACTUAL vision: in the
   open the birds look ~46 units out, but packed flat in a letter they see
   12 — a fixed 48-cell there makes every refresh wade through 13x more
   candidates than it needs (the 95th percentile, so a few far-sighted
   stragglers do not hold the cell wide for everyone) */
let CELL = 48;
const NCELL = 4096;
const visHist = new Int32Array(64);
...
let letT = null, letB = null;          // per-letter world y-bounds (multi-line)
let dealOrd = null;                    // letters sorted by centre-x, for dealing
...
function planWord(text){
  ...
  // a sentence stacks: EVERY whitespace is a line break, each word gets
  // its own centred line — the whole block forms in one summon
  ...
  let asc = 0, desc = 0, wmax = 1;
  const lw = [];
  for (const ln of lines){
    const m = g.measureText(ln);
    lw.push(Math.max(1, m.width));
    asc  = Math.max(asc,  m.actualBoundingBoxAscent  || PROBE*0.78);
    desc = Math.max(desc, m.actualBoundingBoxDescent || PROBE*0.24);
    if (m.width > wmax) wmax = m.width;
  }
  const lh = (asc + desc)*1.16;                          // leading
  const th = lh*(lines.length - 1) + asc + desc;         // block height (probe px)
  const F = Math.min(W*0.80/wmax, H*0.62/th, H*0.28/(asc + desc))*PROBE;
  const SC = F/PROBE;                                    // probe px -> screen px
  // per-line fences between letters, and each line's centring offset,
  // so each glyph can own its share of the flock
  const xoff = lines.map((_, li) => (wmax - lw[li])*SC/2);
  const fen = lines.map(ln => {
    const f = [0];
    for (let k = 1; k <= ln.length; k++) f.push(g.measureText(ln.slice(0, k)).width*SC);
    return f;
  });
  const padPx = F*0.35;                                  // room for the field
  const CP = 4;                                          // grid cell, screen px
  const gw = Math.ceil((wmax*SC + padPx*2)/CP);
  const gh = Math.ceil((th*SC + padPx*2)/CP);
  ...
  for (let li = 0; li < lines.length; li++)
    g.fillText(lines[li], (padPx + xoff[li])/CP, (padPx + (li*lh + asc)*SC)/CP);
  ...
  // interior points grouped BY LETTER (cells claimed line-first by y, then
  // via that line's fences), so the flock can be split equally between the
  // glyphs; each letter's world x AND y bounds are kept so a bird knows
  // when it stands on foreign soil — even a letter on another line
  const perLn = lines.map(ln => [...ln].map(() => []));
  for (let y = 1; y < gh - 1; y++) for (let x = 1; x < gw - 1; x++)
    if (SDF[y*gw + x] < -1.2){
      const pxl = x*CP + CP/2 - padPx;
      const pyl = y*CP + CP/2 - padPx;
      let li = (pyl/(lh*SC))|0;
      if (li < 0) li = 0; else if (li >= lines.length) li = lines.length - 1;
      const fl = fen[li], tx2 = pxl - xoff[li];
      let k = 0;
      while (k < fl.length - 2 && tx2 > fl[k + 1]) k++;
      perLn[li][k].push(SOX + (x + 0.5)*SCELL, SOY + (y + 0.5)*SCELL);
    }
  const own = [], ox0 = [], ox1 = [], oy0 = [], oy1 = [];
  for (const perL of perLn) for (const a of perL){
    if (!a.length) continue;
    let x0 = 1e9, x1 = -1e9, y0 = 1e9, y1 = -1e9;
    for (let t = 0; t < a.length; t += 2){
      if (a[t] < x0) x0 = a[t];       if (a[t] > x1) x1 = a[t];
      if (a[t+1] < y0) y0 = a[t+1];   if (a[t+1] > y1) y1 = a[t+1];
    }
    own.push(a); ox0.push(x0); ox1.push(x1); oy0.push(y0); oy1.push(y1);
  }
  nLet = own.length;
...
function summon(text){
  ...
  // deal the flock into letters by where everyone is RIGHT NOW: sorted by
...
function replan(){
  ...
  for (let i = old; i < nT; i++){
    fw[i] = 0; fd[i] = rnd()*40;
    const h = (rnd()*nHomes)|0;                 // uniform over ink = area share
    let l = 0; while (l < nLet - 1 && h >= homeStart[l + 1]) l++;
    lb[i] = l; hom[i] = h;
  }
}
...
function step(){
  ...
  visHist.fill(0);
  for (let i = 0; i < COUNT; i++) visHist[vis[i]|0]++;
  { let acc = 0, v = 0; const lim = COUNT*0.95;
    for (; v < 63 && (acc += visHist[v]) < lim; v++);
    CELL = Math.max(20, v + 1 + SKIN); }
  head.fill(-1);
  ...
  for (let i = 0; i < COUNT; i++){
    ...
    if (w > 0.001 && SDF){
      ...
      const commuting = x < letL[li] - 12 || x > letR[li] + 12
                     || y < letT[li] - 12 || y > letB[li] + 12;

      // panic opens the walls: a frightened bird bursts OUT of the glyph
      // (and out of its flat plane) instead of rattling inside it — the
      // home pull below is the ticket back once the fear decays
      const wf = w*Math.max(0, 1 - 2.0*fear[i]);
      ...
      const dz0 = z - 1050;
      fz -= Math.max(-0.55, Math.min(0.55, dz0*0.05))*wf;
      vz[i] *= 1 - 0.10*wf;
...
function render(){
  ...
  // dots: one path per colour bucket, filled once — not one call per bird
  if (sheenOn){
  ...
  if (FAL.on){
    ...
    ctx.fillStyle = 'rgba(226,90,80,0.85)';
    ctx.beginPath(); ctx.arc(x, y, 15*s, 0, 6.2832); ctx.fill();
  }
...
/* the words box IS the repertoire: one SENTENCE per line, cycled in
   order — inside a sentence every whitespace stacks the next word on a
   new display line. Applying mid-word drains the standing block first */
const wordsIn = document.getElementById('kb-words');
function applyWords(){
  WORDS = wordsIn.value.split('\n')
    .map(s => s.trim().replace(/\s+/g, ' ')).filter(Boolean);
  wi = 0;
  if (MODE === 'form' || MODE === 'hold') release();
  else if (MODE === 'free') modeT = Math.min(modeT, 40);
}
wordsIn.addEventListener('change', applyWords);        // applies on blur
...
/* dev hook: ?bench=N runs N steps+renders back to back and reports the
   mean cost of each half (ms) in the tab title, split by letter mode */
const mBench = /bench=(\d+)/.exec(location.search);
if (mBench){
  const acc = { free: [0, 0, 0], form: [0, 0, 0], hold: [0, 0, 0], release: [0, 0, 0] };
  for (let i = 0, n = +mBench[1]; i < n; i++){
    const t0 = performance.now(); step();
    const t1 = performance.now(); render();
    const t2 = performance.now(), a = acc[MODE];
    a[0]++; a[1] += t1 - t0; a[2] += t2 - t1;
  }
  document.title = 'BENCH ' + Object.entries(acc).filter(([, a]) => a[0])
    .map(([m, a]) => `${m}:n=${a[0]} sim=${(a[1]/a[0]).toFixed(2)} draw=${(a[2]/a[0]).toFixed(2)}`).join(' | ');
}
...
function loop(now){
  ...
  if ((statT += dt) > 500){ statT = 0; stat.textContent = `${COUNT} birds · ${fps|0} fps · sim ${msStep.toFixed(1)} · draw ${msDraw.toFixed(1)} ms`; }
  ...
  msStep += (t1 - t0 - msStep)*0.05; msDraw += (t2 - t1 - msDraw)*0.05;
}
requestAnimationFrame(loop);
